...
class TaskHttpHandler extends UserHttpHandlerAbstract
{
    ...
    {
        $task = $taskService->getOne((int)$getData['id']);
    ...
    public function delete(TaskServiceInterface $taskService, array $getData = []): void
    {
        if (!isset($getData['id'])) {
            $this->redirect('dashboard.php');
        }

        $taskService->delete((int)$getData['id']);
        $this->redirect('dashboard.php');
    }
     ...
     * @throws \Exception
     */
    private function handleInsertProcess($taskService, $userService, $categoryService, $formData): void
    {
        /** @var TaskDTO $task */
        $task = $this->dataBinder->bind($formData, TaskDTO::class);
        ...
        /** @var CategoryService $categoryService */
        $category = $categoryService->getOne((int)$formData['category_id']);
    ...
    private function handleEditProcess($taskService, $userService, $categoryService, $formData, $getData): void
    {
        try {
            /** @var TaskDTO $task */
            $task = $this->dataBinder->bind($formData, TaskDTO::class);
            ...
            /** @var TaskService $taskService */
            $taskService->edit($task, (int)$getData['id']);
            $this->redirect('dashboard.php');
        } catch (\Exception $ex) {
            echo $ex->getMessage();
        }
